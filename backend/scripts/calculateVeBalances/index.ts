...
task("calculate-ve-balances", "Calculates VE balances at blocknumber")
  ...
  .setAction(async ({ blocknumber }, { network, ethers }) => {
    ...
    const sortedLockSnapshotPretty = Object.entries(lockSnapshotPretty)
      .map(([owner, { lockAmount, lockPercentage }]) => ({
        owner,
        lockAmount,
        lockPercentage,
      }))
      .filter((entry) => entry.lockPercentage > 0)
      .sort((a, b) => b.lockPercentage - a.lockPercentage);
    ...
    fs.writeFileSync(
      totalFilePathPretty,
      JSON.stringify(sortedLockSnapshotPretty)
    );
